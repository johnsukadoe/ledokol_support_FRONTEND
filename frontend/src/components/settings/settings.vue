<script>
import HomepageHeader from "@/components/Homepage/components/homepage-header.vue";
import {getCreator, updateCreator} from "@/api/creators.js";
import {getUser} from "@/api/homepageUser.js";

export default {
  name: "settings",
  components: {HomepageHeader},
  data(){
    return{
      user:{
        username:'',
        channel_description:'',
        user_id:null,
        password:'',
      }
    }
  },
  async mounted(){
    console.log(this.user_id)
    const data = await getCreator(this.user_id);
    const user = await getUser(this.user_id)

    this.user.username = data.username
    this.user.channel_description = data.channel_description;
    this.user.user_id = this.user_id
    this.user.password = user.password
  },
  computed:{
    user_id(){
      return this.$route.params.userId;
    }
  },
  methods:{
    async save(){
      const data = await updateCreator(this.user)
      if(data.status === 200){
        this.$message.success('Успешно')
        this.$router.push({name:'profile', params:{userId:this.user_id}})
      }else{
        this.$message.error("Ошибка")
      }
    }
  }
}
</script>

<template>
  <div style="max-width: 1280px; margin: 0 auto">
    <homepage-header></homepage-header>
    <div class="mt-4">
      <h1>Настройки</h1>
      <div class="flex flex-col gap-5">
        <div class="mt-5">
          <h2>Название канала</h2>
          <el-input v-model="user.username"></el-input>
        </div>
        <div>
          <h2>Описание канала</h2>
          <el-input v-model="user.channel_description" type="textarea"></el-input>
        </div>
        <div>
          <h2>Пароль канала</h2>
          <el-input
              v-model="user.password"
              type="password"
              placeholder="Please input password"
              show-password
          />
        </div>
      </div>
      <div class="mt-6">
        <el-button type="danger" plain round @click="$router.push({name:'profile', params:{userId:user_id}})">Отменить</el-button>
        <el-button type="primary" plain round @click="save">Сохранить</el-button>
      </div>
    </div>
  </div>
</template>

<style scoped lang="scss">
h1{
  font-size: 32px;
  font-weight: 600;
}
h2{
  font-size: 17px;
  font-weight: 500;
}
</style>