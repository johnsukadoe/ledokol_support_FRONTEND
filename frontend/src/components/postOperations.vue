...
export default {
  ...
  async mounted() {
    if(this.isEdit){
      this.title = this.lang === 'ru' ? 'Редактирование поста' : 'Editing the post';
  ...
  methods:{
    async createPost(){
      ...
      if(this.post.titleEN.length===0 || this.post.titleRU.length === 0 || this.post.descriptionEN.length ===0 || this.post.descriptionRU.length === 0){
  ...
  <div class="my-4">
    ...
    <div class="flex justify-around">
      <div class="flex flex-col justify-start items-start align-middle gap-4 my-4">
        <el-input v-model="post.titleRU" placeholder="Заголовок поста" style="width: 700px;" size="large" required></el-input>
        <el-input v-model="post.titleEN" placeholder="Title of the post" style="width: 700px;" size="large" required></el-input>
      ...
      <div style="width: 300px; " >
        ...
        <h2 class="text-2xl font-medium">
          {{ lang === 'en' ? 'Bring something of your own into the world' : 'Принеси в мир что-то свое' }}
        </h2>
      </div>
    </div>
  </div>
